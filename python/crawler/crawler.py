...
def is_valid_keyword(keyword):
    ...
    if not (1 < len(keyword) < 50): return False
    excluded_keywords = [
        ':', '코스', '가격', '정보', '시간', 'Day', 'DAY', '일차', '여행 팁', '참고', '주의',
        '준비물', '꿀팁', '가는 법', '예상 비용', '예약', '후기', '추천', '만족', '솔직',
        '문의', '위치', '주소', '전화', '영업', '휴무', '주차', '입장료'
    ]
    if any(kw in keyword for kw in excluded_keywords): return False
    sentence_patterns = [r'.*다\.?$', r'.*요\.?$', r'.*죠\.?$', r'.*음$', r'.*함$', r'.*에서$', r'.*으로$', r'.*에게$', r'.*(와|과)$', r'.*(은|는|이|가)$']
    if any(re.match(pattern, keyword) for pattern in sentence_patterns) and not keyword.endswith("가"): return False
    if keyword.isdigit() or (sum(c.isdigit() for c in keyword) / len(keyword) > 0.5): return False
    if re.search(r'[!@#$%^&*()=+/\\|?><]', keyword): return False
    return True

def verify_place_with_kakao_api(place_name):
    """카카오 API로 장소 이름이 실제 서울에 있는지 확인"""
    if not KAKAO_API_KEY or len(KAKAO_API_KEY) != 32: return False
...
def save_itinerary_to_db(itinerary, blog_url, owner_id=1):
    ...
    try:
        ...
        cursor.execute("SELECT trip_id FROM TRIPS WHERE shareable_link_id = %s", (shareable_link_id,))
        ...
        cursor.execute("INSERT INTO TRIPS (trip_name, owner_id, shareable_link_id) VALUES (%s, %s, %s)", (trip_name, owner_id, shareable_link_id))
        new_trip_id = cursor.lastrowid
        print(f"  - 새로운 여행 생성: '{trip_name}' (Trip ID: {new_trip_id})")
        cursor.execute("INSERT IGNORE INTO COLLABORATORS (trip_id, user_id) VALUES (%s, %s)", (new_trip_id, owner_id))
        ...
        for day_str, places in itinerary.items():
            ...
            for place_name_candidate in places: # 추출된 모든 후보를 순회
                ...
                if verify_place_with_kakao_api(place_name_candidate):
                    ...
                    # 검증 성공 시에만 DB 작업 수행
                    cursor.execute("SELECT place_id FROM PLACES WHERE place_name = %s", (place_name_candidate,))
                    result = cursor.fetchone()
                    
                    if result:
                        place_id = result[0]
                    else:
                        cursor.execute("INSERT INTO PLACES (place_name) VALUES (%s)", (place_name_candidate,))
                        place_id = cursor.lastrowid
                        print(f"    - 신규 장소 추가: '{place_name_candidate}' (ID: {place_id})")

                    cursor.execute(
                        "INSERT IGNORE INTO ITINERARY_ITEMS (trip_id, place_id, user_id, visit_day_str, order_in_day) VALUES (%s, %s, %s, %s, %s)",
                        (new_trip_id, place_id, owner_id, day_str, current_order) # order 대신 current_order 사용
                    )
                    ...
                    # 검증 실패 시 로그 출력 (선택적)
                    print(f"  [저장 거부] '{place_name_candidate}' (유효하지 않거나 서울 장소 아님)")
            # --- --- ---
...
def extract_itinerary_from_content(content_html):
    ...
    itinerary = {}
    day_pattern = re.compile(r'(Day\s*\d+|[0-9]+\s*일차|첫째\s*날|둘째\s*날|셋째\s*날)', re.IGNORECASE)
    if not day_pattern.search(str(content_html)): return None
    ...
    for section in day_sections:
        if day_pattern.match(section):
            ...
            if day_number_match:
                ...
                if current_day_key not in itinerary: itinerary[current_day_key] = []
            ...
            for tag in potential_tags:
                ...
                if is_valid_keyword(cleaned_place_name) and cleaned_place_name not in processed_in_section:
...
def extract_places_by_verification(content_html):
     ...
     for tag in potential_tags:
         ...
         if is_valid_keyword(cleaned_keyword) and cleaned_keyword not in processed_candidates:
             place_candidates.append(cleaned_keyword)
             processed_candidates.add(cleaned_keyword)
             
     if place_candidates: return {"방문 코스": place_candidates}
     return {}
...
def get_blog_content(blog_url):
    ...
    itinerary = None
    try:
        options = webdriver.ChromeOptions(); options.add_argument('headless'); options.add_argument('window-size=1920x1800')
        options.add_argument("user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/5.0 (KHTML, like Gecko) Chrome/98.0.4758.102 Safari/537.36")
        driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=options)
        driver.get(blog_url); time.sleep(4)
        content = None
        if "blog.naver.com" in blog_url:
            try: driver.switch_to.frame('mainFrame'); time.sleep(1)
            except: print("Naver mainFrame 전환 실패"); return 
            ...
            html = driver.page_source
            soup = BeautifulSoup(html, 'html.parser')
            content = soup.select_one('.tt_article_useless_p_margin, .entry-content, [role=main], .contents_style')
        
        if content:
            itinerary = extract_itinerary_from_content(content)
            if not itinerary: itinerary = extract_places_by_verification(content)
            
            if itinerary:
                print("\n--- 🗺️ 추출된 장소 후보 목록 ---"); print(json.dumps(itinerary, indent=2, ensure_ascii=False)); print("--------------------------")
