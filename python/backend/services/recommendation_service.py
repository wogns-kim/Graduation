...
TAG_TO_CATEGORY_MAP = {
    ...
    # "액티비티": ["스포츠", "테마파크", "레저"],
    "액티비티": ["방탈출카페", "영화관", "빌딩", "게임,PC방"],
    # "문화/역사": ["문화유산", "궁궐", "박물관", "미술관", "역사"],
    "문화/역사": ["문화", "문화유적", "문화시설", "박물관", "고궁", "사회,공공기관"],
    ...
    # "미식/쇼핑": ["음식점", "맛집", "쇼핑", "시장"],
    "미식/쇼핑": ["음식점", "맛집", "쇼핑", "시장", "카페", "음식점"],
    # 식사 선호도
    "로컬 맛집": ["음식점", "아시아음식", "시장", "멕시칸,브라질"],
    "가성비 맛집": ["일식", "분식", "시장"],
    "프리미엄": [
        "술집",
        "양식",
    ...
    "나 홀로 여행": ["패션", "제과,베이커리", "카페", "공원"],
    "친구/지인": ["카페", "문화,예술", "가정,생활", "취미"],
    "가족 여행": ["가정,생활", "관광,명소"],
    "펫 동반 여행": ["공원", "호수", "하천", "계곡"],
    "아이 동반 여행": ["테마파크", "어린이", "가족공원", "키즈", "계곡"],
}
    ...
    if db_keywords:
        try:
            ...
            like_conditions = " OR ".join(
                [f"PLACES.category LIKE :kw{i}" for i, kw in enumerate(db_keywords)]
            )
...
def nearest_neighbor_route(
    places: List[PlaceSchema], start_location: Tuple[float, float] = None
...
def get_optimized_route_by_categories(
    ...
    places_orm = db.query(Place).filter(Place.category.in_(categories)).all()

    # ORM 객체를 Pydantic 모델 리스트로 변환
    places = [PlaceSchema.model_validate(p) for p in places_orm]

    if len(places) < 2:
        return RouteResponse(theme=f"장소 부족 ({', '.join(categories)})", route=[])
